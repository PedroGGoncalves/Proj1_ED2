	...
	{	
		...
		if (((int)codfilme)=='*') //se for encontrado o marcador de remoção
		{
			fseek(out,0,0); //coloca arquivo no inicio do arquivo
   			fread(&codfilme,sizeof(char),1,out); //le o tamanho que será necessario se mover até o proximo
		    ...
		    {
		   	 	fseek(out,((int)codfilme),0); //locomove para a posição codfilme 
				fread(reg1,sizeof(reg1),1,out); //todos dados até então serão salvos
				fclose(out);
				if ((out = fopen("saida.bin","w+b")) == NULL)//o arquivo é reaberto com o objetivo de reescrever ele do começo
				{
			   	...
			   	fwrite(reg, sizeof(char), strlen(reg), out); //o que foi salvo no registrador reg será escrito//
			   	int cont=1;
			   	while(cont<=(((int)codfilme)-temp_2)) //preenche  '-' a qtd de vezes que é obtido em b
	   			{
	   				sprintf(reg,"%c",'-');
				...
				{
					//fseek(out,-1,SEEK_CUR);
					fwrite(reg1, sizeof(char), strlen(reg1), out);//salva isso no reg1 no equivalente ao espaço sobrando//
				   	fseek(out,-1,SEEK_CUR); //volta 1 posição para substituir a temp_2//
				   	temp_2=temp; //temp_2 terá o valor da temp (valor que ajuda a se mover no arquivo insere)
